...
export default async ({ 
	...
	sections.push({
		items: [
			...
			sectionList([
				'Analysis of construction documents to determine whether energy efficient measures have been incorporated into the design of the building\'s lighting systems',
				...
				'Calculations of deductions available based on the Lighting Power Density reduction calculation of the subject building.',
				`Use of an energy analysis software to calculate the Lighting Power Density (LPD) of the building and an analysis to calculate the LPD reduction versus the American Society of Heating, Refrigeration and Air Conditioning Engineers (ASHRAE) Standard 90.1-${year} reference values.`
			])
		]
	})
	// sections.push({
	...
	sections.push({
		items: [
			...
			sectionParagraph(`Based on the Lighting Power Density reduction calculations, the Lighting Systems will qualify as Energy Efficient Commercial Building Property. Therefore, the property will qualify for a deduction of $0.60 per square foot, limited to the cost of the qualifying systems. This calculation is based upon a total combined square footage of ${formatNumber(totalBuildingArea)}.`),
			sectionTable({
				columns: [{
					type: 'string',
					header: 'Name',
	...
	sections.push({
		items: [
			...
			.concat(
				(photos.length > 0) ? 
					[
						sectionSubtitle('a. Site Inspection Photographs'),
